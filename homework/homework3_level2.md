...
//本程序采取n*5的方阵的输出形式，为了方便后续处理，首先用一个列表c对一列的所有的可能的形势进行按二进制顺序的编号。方便输出和图像处理，比如乘二左移，除二右移。

//根据c的编号用n个0到31的数表示要输出的符号，范例程序为了简单采取n=5的情况，并只对大写字母编号。建立一个这样的字典后方便以后调用。

//从键盘读取输入，并初始化输出。

//把输出内容的每一行加载到分行输出的列表的每一行里。
